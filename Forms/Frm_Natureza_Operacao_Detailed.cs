...
{
    public partial class Frm_Natureza_Operacao_Detailed : Form
    {
        public static Frm_Natureza_Operacao_Detailed instance;
        cls_mysql_conn connection = new cls_mysql_conn();
        cls_populate_views populate = new cls_populate_views();
        
        public Frm_Natureza_Operacao_Detailed()
        {
            InitializeComponent();
        ...
        private void ListarDetalhado()
        {
            try
            {
                ...
                string sql = "select DISTINCT a.ENTRADA_SAIDA, a.TIPO, a.NUMERO_DA_NOTA, a.CHAVE_ACESSO, a.RAZAO_SOCIAL, a.DTA_EMISSAO from db_sis.tb_conf_ndd a inner join db_sis.tb_conf_c5 b on a.CHAVE_ACESSO = b.CHAVE_ACESSO where a.COD_CLIENTE = @COD_CLI AND a.COD_EMPRESA = @COD_EMP AND a.MES = @MES AND a.ANO = @ANO AND b.CFOP = @CFOP AND a.NATUREZA_OPERACAO = @NAT";
                ...
                populate.PopulateListViews(lsv_nat_detalhado, cmd, columns); 
            }
            catch (Exception ex)
            {
        ...
        private void Frm_Natureza_Operacao_Detailed_Load(object sender, EventArgs e)
        {
            try
            {
                ListarDetalhado();
